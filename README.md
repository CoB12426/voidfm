...
| cmake / build-essential | Linux のみ | `sudo apt install cmake build-essential` |
...
`easy_up_all` 実行時に s2.cpp の自動ビルドと tokenizer.json のコピーが行われます。  
GGUF モデルのみ手動で配置が必要です：

```bash
mkdir -p models
# s2-pro-q4_k_m.gguf を models/ に配置
```
