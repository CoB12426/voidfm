...
```powershell
git clone https://https://github.com/CoB12426/voidfm.git
cd voidfm
...
> GPU を使わない場合は `-f docker-compose.gpu.yml` を除いてください。
...
ステップ 3 の起動コマンドは NVIDIA GPU を前提としています。
...
GPU がない場合は、ステップ 3 のコマンドから `-f docker-compose.gpu.yml` を除いて実行してください（動作しますが、トーク生成が遅くなります）。
