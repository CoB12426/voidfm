...
# DJ トークは英語専用
_OUTPUT_INSTRUCTION = "Deliver the airbreak in natural English."
...
def _get_context() -> str:
    ...
    return (
        f"[Live context — let this inform your TONE silently; do NOT mention the time/date "
...
async def build_prompt(
    ...
    logger.debug(
        "Built prompt (personality=%s, mid_song=%s): %s",
        personality, is_mid_song, prompt,
    )
    return prompt
...
def _build(
    ...
    guidelines = (
        ...
        f"You are {persona}.\n"
        f"{style}\n\n"
        f"The music is currently paused. {output_instruction}\n"
