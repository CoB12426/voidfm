...
class MainActivity : FlutterActivity() {
    ...
    override fun onActivityResult(requestCode: Int, resultCode: Int, data: Intent?) {
        ...
        localScope.launch { localDj.onPicked(requestCode, resultCode, data) }
    }
            ...
            .setMethodCallHandler { call, result ->
                when (call.method) {
                    "status" -> result.success(mapOf(
                        "hasModel" to localDj.hasModel(),
                        "hasTtsModel" to localDj.hasTtsModel(),
                        "ttsModelPath" to localDj.ttsModelPath(),
                        "ready" to localDj.isReady(),
                        "modelLoaded" to localDj.isModelLoaded(),
                    ))
                    "pickModel" -> localDj.pickModel(result)
                    "pickTtsModel" -> localDj.pickTtsModel(result)
                    "cancel" -> { localDj.cancel(); result.success(null) }
                    "initialize", "generate", "stationId", "unload" -> localScope.launch {
                        try {
                            when (call.method) {
                                "initialize" -> { localDj.initialize(); result.success(true) }
                                "unload" -> { localDj.unload(); result.success(null) }
                                "generate" -> result.success(localDj.generate(call.arguments as? Map<*, *> ?: emptyMap<String, Any>()))
                                else -> result.success(localDj.stationId())
                            }
                        } catch (e: Exception) {
                            result.error("LOCAL_DJ", e.message ?: e.toString(), null)
                        }
                    }
    ...
    private fun showNotificationAccessDialog() {
        AlertDialog.Builder(this)
            ...
            .setMessage("VoidFM needs notification access to read what's playing. Open settings now?")
