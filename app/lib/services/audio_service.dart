...
class AudioService {
  ...
  Future<void> playJingleThenTalk(
    ...
    if (_player.playing) {
      debugPrint(
          'AudioService: playJingleThenTalk — stopping current playback');
      await _player.stop();
    }
    _loadedAssetPath = null;
    try {
      final source = ConcatenatingAudioSource(children: [
        ...
        BytesAudioSource(talkBytes, contentType: contentType),
      ]);
  ...
  Future<void> playTalk(Uint8List wavBytes,
    ...
    try {
      final source = BytesAudioSource(wavBytes, contentType: contentType);
