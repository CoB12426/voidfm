...
class DjProvider extends ChangeNotifier {
  ...
  /// 再生中のトーク種別（'DJ talk' / 'Station ID' / 'Radio jingle'）。
  String? get playingLabel => _playingLabel;

  /// 直前のトーク枠で何が起きたか（再生した / 間に合わずスキップした）。
  String? get lastTalkNote => _lastTalkNote;

  /// 生成中のトーク種別。
  bool get isPreparingStationId => _isGeneratingTalk && _prependJingle;
  ...
  DjBackend? _ttsBackend; // 進行中の生成のキャンセル用
  ...
  // ── 設定 ─────────────────────────────────────────────────────────────
  BackendConfig _backend = const BackendConfig(mode: DjBackendMode.remote);
  ...
  Future<void> toggleService({
    ...
    if (!_isOnAir) {
      ...
      if (_backend.isRemote) {
        ...
        // 端末内モデルはロードに時間がかかるため、イントロ前に準備を済ませる
        final backend = _backend.create();
        final ready = await backend.ping();
        backend.close();
        _hostConnected = ready;
        if (!ready) {
          unawaited(_unloadLocalModels());
          _isOnAir = false;
          _lastTalkError = 'Select Gemma 4 E2B and Supertonic 3 models';
  ...
  Future<void> _unloadLocalModels() async {
    try {
      await LocalDjClient.unload();
    } catch (e) {
      debugPrint('DjProvider: local model unload error: $e');
    }
  }
  ...
  void _noteNotReady() {
    final kind = _prependJingle ? 'Station ID' : 'DJ talk';
    _lastTalkNote =
        'Skipped: $kind was not ready within ${_ttsReadyTimeout.inSeconds}s';
    notifyListeners();
  }
  ...
  void _cancelTts() {
    // 進行中の生成を強制中断する（リモートは HTTP も切断される）
    final backend = _ttsBackend;
    if (backend != null) {
  ...
  Future<void> _playVoidTalk(
    ...
    _playingLabel = withJingle ? 'Station ID' : 'DJ talk';
    _lastTalkNote = null;
    _isPlayingTalk = true;
    _setRuntimeState(DjRuntimeState.playingTalk);
    _lastTalkError = null;
    ...
    try {
      ...
      if (withJingle) {
        ...
        await _audio.playJingleThenTalk('assets/audio/jingle.m4a', wavBytes,
            contentType: _backend.audioContentType);
        debugPrint('DjProvider: _playVoidTalk — jingle + station ID done');
      } else {
        debugPrint('DjProvider: _playVoidTalk — playTalk()');
        await _audio.playTalk(wavBytes, contentType: _backend.audioContentType);
  ...
  /// バックエンドで台本と音声を生成する（null = 失敗 or キャンセル）。
  Future<TalkFetchResult?> _generateTts({
    ...
    return _runBackend('TTS', (backend) async {
      final result = await backend.fetchTalk(
  ...
  /// Station ID の音声バイト列を返す（null = 失敗 or キャンセル）。
  Future<Uint8List?> _fetchStationId() {
    return _runBackend('station ID',
        (backend) => backend.fetchStationId(preferences: _preferences));
  }

  Future<T?> _runBackend<T>(
      String label, Future<T> Function(DjBackend backend) task) async {
    final backend = _backend.create();
    _ttsBackend = backend;
    try {
      return await task(backend);
    } catch (e) {
      if (_ttsBackend != backend) {
        // _cancelTts() による中断
        debugPrint('DjProvider: $label cancelled: $e');
        return null;
      }
      debugPrint('DjProvider: $label failed: $e');
      _lastTalkError = e.toString();
      notifyListeners();
      return null;
    } finally {
      // 自分が登録したバックエンドなら解放
      if (_ttsBackend == backend) _ttsBackend = null;
      backend.close();
    }
  }
  ...
  void _pingHostSilently() {
    if (_backend.hostAddress.isEmpty) return;
    final backend = _backend.create();
    backend.ping().whenComplete(backend.close).then((ok) {
